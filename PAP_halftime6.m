...
%        off=off(2:end);
    ...
    for j=1:noff
     ...
     for j=1:non
         ...
         if j+1<=noff
            ...
            Pon=cat(1, Pon, lsqnonlin(modelOn,[mean(Y(off(j):on(j))) mean(Y(on(j):Nframes)) on(j) 10],[],[],[],x(on(j)-area1:min(on(j)+area1,Nframes)),Y(on(j)-area1:min(on(j)+area1,Nframes))));
         end
     end
     for j=1:noff
 ...
 ylabel('Brightness, AU')
 % plot(x,Y,'r');
 plot(x(100:end-10),Y(100:end-10),'g'); % plots without first 100 and last 10 frames
 for j=1:non
    dataL=max(1,on(j)-area1);
    dataR=min(Nframes,on(j)+area1);
 ...
 %% ignore last on- and off-switching if it might be corrupt (-> deviates more than factor 1.5 from mean of rest)
