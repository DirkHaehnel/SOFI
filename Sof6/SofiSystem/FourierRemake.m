...
mag = 100;%handles.vMicMagnification; % magnification
pix = mag*0.100/4; % virtual pixel size in micron
...
% imageSofiFourierModInitForAnalysis = baseSofiXFourierMod;
 ...
 for j=1:nz
     ...
     otm2 = otm2/max(otm2(:));
     dd = 3;
     otm3 = interp2((-(dd)*myInf:(dd):(dd)*mySup)',-(dd)*mxInf:(dd):(dd)*mxSup,otm1,(-myInf:mySup)',-mxInf:mxSup,'cubic');
...
externDirectory = 'C:\Users\FaslforIDL\Desktop\DEMENCIAL\';
for j=1:size(imageSofiFourier,3)
    ...
    imwrite(imageSofiFourier(:,:,j),strcat(externDirectory,externActualFile,'_2SofiFourier',int2str(j+1),'.tif'))
    ...
    imwrite(imageSofiFourierMod(:,:,j),strcat(externDirectory,externActualFile,'_2SofiFourierMOd',int2str(j+1),'.tif'))
    ...
    imwrite(imageSofiFourierModInit(:,:,j),strcat(externDirectory,externActualFile,'_2SofiFourierMOdInit',int2str(j+1),'.tif'))
...
%     imwrite(imageSofiFourierForAnalysis(:,:,j),strcat(externDirectory,externActualFile,'_3SofiFourierForAnalysis',int2str(j+1),'.tif'))
%     
...
%     imwrite(imageSofiFourierModForAnalysis(:,:,j),strcat(externDirectory,externActualFile,'_3SofiFourierMOdForAnalysis',int2str(j+1),'.tif'))
% 
...
%     imwrite(imageSofiFourierModInitForAnalysis(:,:,j),strcat(externDirectory,externActualFile,'_3SofiFourierMOdInitForAnalysis',int2str(j+1),'.tif'))

end

save(strcat(externDirectory,'Data_',externActualFile,'_ProccesedData','.mat'))
clear imageSof*