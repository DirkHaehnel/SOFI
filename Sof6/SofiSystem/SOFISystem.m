                ...
                if(handles.vUseMDataFiles == 0)
                    ...
                    for i=1: dimTF(1)
                        if(~tempFiles(i).isdir)
                            noValid = 0;
                            zText = regexp(tempFiles(i).name, '_AverageImage', 'match');
                            dummy = size(zText);
                            noValid = noValid + dummy(1);
                            regexp(tempFiles(i).name, '_MaxImage', 'match');
                            dummy = size(zText);
                            noValid = noValid + dummy(1);
                            regexp(tempFiles(i).name, '_ExtendedImage', 'match');
                            dummy = size(zText);
                            noValid = noValid + dummy(1);
                            regexp(tempFiles(i).name, '_1Sofi', 'match');
                            dummy = size(zText);
                            noValid = noValid + dummy(1);
                            regexp(tempFiles(i).name, '_2SofiFourier', 'match');
                            dummy = size(zText);
                            noValid = noValid + dummy(1);
                            if(noValid < 1)
